...
function buildDynamoObject(tag, revalidatedAt) {
    return {
...
export default {
    ...
    hasBeenRevalidated: async (tags, lastModified) => {
        ...
        const revalidatedTags = Responses[CACHE_DYNAMO_TABLE ?? ""].filter((item) => Number.parseInt(item.revalidatedAt.N) > (lastModified ?? 0));
